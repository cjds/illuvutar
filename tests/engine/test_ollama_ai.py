...
from illuvutar.engine.entities.components import Position, Label, AIComponent, Tags, PhysicsComponent
...
async def test_ai_schedules_think_at_interval(ai_system):
    ...
    with patch("illuvutar.engine.systems.ollama_ai.ollama") as mock_ollama:
    ...
    from illuvutar.engine.wrl.schema import WRLThought
...
async def test_decision_rewrites_memory_facts_goal_and_persists(store_with_mind, passability, tmp_path):
    ...
    # persisted to disk
    from illuvutar.engine.entities.persistence import load_entity_state
...
from illuvutar.engine.entities.components import Profile
