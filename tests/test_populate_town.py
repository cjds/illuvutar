...
def test_populate_town_survives_malformed_regions(tmp_path):
    tools, writer = _tools(tmp_path)
    writer.write("regions", {"regions": [{"name": "x"}, 5, {"id": "bad", "biome": "grassland"}]})
    writer.write("tilemap", [{"x": x, "y": 0, "tile_id": "grass_plain", "region": 0} for x in range(20)])
    with patch("illuvutar.generation.populace.ollama") as mo:
        m = MagicMock(); m.message.content = json.dumps(
            {"name": "N", "backstory": "b", "goal": "g", "facts": "f"})
        mo.chat.return_value = m
        out = tools.populate_town(count=3)
    assert "error" not in out.lower()          # clean success, no exception surfaced
    assert len(writer.read("agents")) == 3
