...
from illuvutar.god.llm.client import LLMClient, parse_json
...
def test_chat_returns_plain_text():
    with patch("illuvutar.god.llm.client.OpenAI", return_value=_fake_openai(content="hello")):
...
def test_chat_normalizes_tool_calls_to_dict_args():
    ...
    with patch("illuvutar.god.llm.client.OpenAI", return_value=_fake_openai(content="", tool_calls=[tc])):
...
def test_config_precedence_env_then_default(monkeypatch):
    ...
    with patch("illuvutar.god.llm.client.OpenAI") as O:
        LLMClient()
...
def test_complete_returns_plain_text():
    with patch("illuvutar.god.llm.client.OpenAI", return_value=_fake_openai(content="hi there")):
